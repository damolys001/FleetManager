...
<?php include_once "../resource/session.php"; $_SESSION["prodidforUpdate"]= "";$_SESSION["ref"]= ""; ?>
<!DOCTYPE html>
<html lang="en"  ng-app="fleetEdit"><!-- InstanceBegin template="/Templates/templateMain.dwt.php" codeOutsideHTMLIsLocked="false" -->
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                ...
                <a href="fleet.php">Fleet</a>/ <span id="title"></span>  
                    ...
                    <div class="col-lg-6 ">
                     ...
                     <div class="col-lg-6 ">
                      ...
                      <label > Chasis Number</label>
                 		<div class="form-group input-group">
                      	<span class="input-group-addon"></span>
                        <input id="chasisNo" type="text" readonly   value=""  class="form-control">
                        <span class="input-group-addon"></span> 
...
////////////////////////////////////GET URL PARAMETERS FUNCTION
    ...
    $.ajax({
                    ...
                    success:function(data)
                    {
                       ...
                       // $("#model").val();
                      $("#colour").val(d.color);
                      $("#chasisNo").val(d.chasisNo);
                     $("#engineNo").val(d.engineNo);
                    ...
                    $("#year").val(d.year);
                      ...
                      //////////handle the make dropdown  
                        $.ajax({
                            type:"POST",
                            url:"../resource/fleetmakeGetAll.php?f=j",
                            success:function(j)
                            {

                                var options = '';
                                for (var i = 0; i < j.length; i++) {
                                    if (j[i].fleetmake.toLowerCase() ==d.maker.toLowerCase())
                                    {
                                         options += '<option selected value="' + j[i].fleetmake + '">' + j[i].fleetmake + '</option>';
                                    }else
                                    {
                                options += '<option value="' + j[i].fleetmake + '">' + j[i].fleetmake + '</option>';
                                  }
                                }
                        ...
                        $.ajax({
                            type:"POST",
                            url:"../resource/fleetmodelGetAll.php?make="+make,
                            data:"make="+make,
                            success:function(df)
                            {
                                ...
                                for (var i = 0; i < df.length; i++) {
                                    if (df[i].fleetmodel.toLowerCase() ==d.model.toLowerCase())
                                    {
                                         options += '<option selected value="' + df[i].fleetmodel + '">' + df[i].fleetmodel + '</option>';
                                    }else
                                    {
                                        options += '<option value="' + df[i].fleetmodel + '">' + df[i].fleetmodel + '</option>';
                                    }
                                }
                       ...
                       //////////handles the department dropdown  
                        ...
                        $.ajax({
                            ...
                            success:function(df)
                            {
                                ...
                                for (var i = 0; i < df.length; i++) 
                                {
                                    if (df[i].id ==d.DriverId)
                                    {
                                         options += '<option selected value="' + df[i].id + '">' + df[i].fName+" "+ df[i].lName+ '</option>';
                                    }else
                                    {
                                        options += '<option value="' + df[i].id + '">' + df[i].fName+" "+ df[i].lName + '</option>';
                                    }
                                }
                                  $("select#driver").html(options);
                        ...
                        //////////handles the fleet status dropdown  
                        $.ajax({
                            type:"POST",
                            url:"../resource/fleetStatusGetAll.php?f=j",
                            data:"f=j",
                            success:function(df)
                            {   var options = '';
                                for (var i = 0; i < df.length; i++) {
                                    if (df[i].id ==d.status)
                                    {
                                         options += '<option selected value="' + df[i].id + '">' + df[i].fleetStatus+ '</option>';
                                    }else
                                    {
                                        options += '<option value="' + df[i].id + '">' + df[i].fleetStatus + '</option>';
                                    }
                                }
                                  $("select#fleetStatus").html(options);
...
$(document).ready(function() {
	...
	{	
	   ...
	   var myp = "../resource/fleetPreImgUpload.php?type=fleet&regNo=" + regNo+"&chasisNo="+ chNumber;
		       $.ajax({
            type:"GET",
            url: myp  ,
            success:function(data){	} });
...
$(document).on("click","a.picdeleteid", function () 
	{
      ...
      confirmButtonColor: "#ec6c62"
        ...
        $.ajax(
                {
                    type: "get",
                    url:"../resource/uploadDelete.php?DelId=0&type=fleet" ,
							success:function(data){
