...
$statename = $_GET["name"];
$detail = $_GET["detail"];
$stateid = $_SESSION ["stateUpdatingId"];
...
if ($stateid == ""){
	...
	if ($statename == "" ) {}else {
		...
		$queryCheck = "select * from state where state = '$statename'";
		...
		if(mysqli_num_rows($result)) 
		{
			...
			{
				$query = "update state set state = '$statename', detail ='$detail' , updated= '$datenow', updatedby ='$name' where state ='$state'";
				$mode ="updated";
			}
		}
		else
		{
				...
				$query  = "INSERT INTO `state` (
				 `id`,  `state`, `detail`,  `created`, `createdby`
				  ) 
				  VALUES 
				  (NULL,
				  '$statename', 
	...
	//$adminPass = sha1 ($defaultPass);
	//$adminPassSalt = sha1 ($email.'_'.$defaultPass);
	$query = "update state set state = '$statename', detail ='$detail' , updated= '$datenow', updatedby ='$name' where id ='$stateid'";
						$mode ="updated";
	}
