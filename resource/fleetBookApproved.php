...

{
      ...
      //check if user have an approved booking
$queryBkHr = "select * from  bookinglog where   user = '".$fleetUser."' and status = 'Approved' and createdForD ='".$datenow."'" ;
   // echo $queryBkHr; and 
...
if(mysqli_num_rows($resultBkHr)) 
	{			
		...
		{
             //echo ("This user have an approved booking request between ".$row["start"]. " and ".$row["end"].". You must cancel or end trip before you can approve another." );
            echo ("This user have an approved booking. You must cancel or end trip before you can approve another." ); //time selected is not available a booking exist for the selected car between ".$row["start"]. " and ".$row["end"]."
     exit;
        }
    }
    ...
    $querAuthoriszed ="update bookinglog set status ='Approved', approved='$datenow', approvedBy ='".$_SESSION["userid"]."' where id ='$id' ";
                ...
                { 
                  //domail to admin
                    ...
                    Dear Admin,<br/><br/>
Kindly note that an <span style='color:red'>$tripType</span> booking request for the car detailed below had been approved<br/><br/>
<table align='center' style='border-collapse: collapse;    width: 50%; tr:nth-child(even):background-color: #f2f2f2}'>
<tr><td>Car Make:</td><td> $carMakeModel<td></tr>
<tr><td>Car User:</td><td> $fleetuserName<td></tr>
<tr><td>Car Registration No: </td><td>$regNo<td></tr>
...
Kindly note that an <span style='color:red'>$tripType</span> booking request for the car detailed below had been approved by $userName<br/><br/>
<table align='center' style='border-collapse: collapse;    width: 50%; tr:nth-child(even):background-color: #f2f2f2}'>
<tr><td>Car Make:</td><td> $carMakeModel<td></tr>
<tr><td>Car User:</td><td> $fleetuserName<td></tr>
<tr><td>Car Registration No: </td><td>$regNo<td></tr>
...
<tr><td>Driver Phone:</td><td>$driverPhone<td></tr>
                   ...
                   {
                         $alert->DoMail($adminEmail,"Fleet Manager",$mailBody,"Inter State Car Booking Approval",$link);
                      
                   }else
                   {
                         $alert->DoMail($adminEmail,"Fleet Manager",$mailBody,"Car Booking Approval",$link);
                    ...
                    //do sms to driver
                    $smsBody ="You have been schedule for a trip. Pickup location:$depature, Destination:$destination, Time/Date:".$myStartDateTime->format('h:i A')." End:".$myEndDateTime->format('h:i A').". By:$fleetuserName";
...

{
    $querAuthoriszed ="update bookinglog set status ='Cancelled', approved='$datenow', approvedBy ='".$_SESSION["userid"]."' where id ='$id' ";
                if ($conn->query($querAuthoriszed) === TRUE) 
                {
                    ...
                    $mailBodyUser="<style>table {border-collapse: collapse;    width: 50%;} th, td { text-align: left; padding: 8px;} tr:nth-child(even){background-color: #f2f2f2}</style>
                    Dear $fleetuserName,<br/><br/>
Kindly note that the booking request has been canceled by $userName <br/><br/>
<table align='center' style='border-collapse: collapse;    width: 50%; tr:nth-child(even):background-color: #f2f2f2}'>
<tr><td>Car Make:</td><td> $carMakeModel<td></tr>
<tr><td>Car User:</td><td> $fleetuserName<td></tr>
<tr><td>Car Registration No: </td><td>$regNo<td></tr>
...
<tr><td>Driver Phone:</td><td>$driverPhone<td></tr>
                    ...
                    ";
                    $alert->DoMail($adminEmail,"Fleet Manager",$mailBodyUser,"Booking Request Cancellation",$link);
                    
                    //$alert->DoMail('damola.adeyemi@ensure.com.ng',"Fleet Manager",$mailBodyUser,"Booking Request Cancellation",$link);
                    
                    //do sms to driver
                    $smsBody ="Your scheduled trip has been canceled for Pick location:$depature, Destination:$destination, Time/Date:".$myStartDateTime->format('h:i A').". By: $userName ";
...
if ($opr=="ext")
{
    ///check if booking if 2hr to the end off booking time $date->getTimestamp()
    if ($now < $myEndDateTime->getTimestamp() - 3600)//$myEndDateTime->getTimestamp()
    {
        echo "You can't extend trip until one hour to the end of the trip.";
         $alert->DoMail("damola.adeyemi@ensure.com.ng",$fleetuserName,"You can't extend trip until one hour to the end of the trip. time now ".$now." ". $date->getTimestamp().$date->format('d/m/Y h:i A')." end time:".$myEndDateTime->getTimestamp()." end time ".$myEndDateTime->format('d/m/Y h:i A')," Car Booking Extension",$link);
        exit;
    ...
    $querAuthoriszed ="update bookinglog set end ='".$datef->format('Y/m/d H:i')."', endDate='".$datef->format('Y/m/d')."', endTime ='".$datef->getTimestamp()."' where id ='$id' ";
                ...
                { 
                  //do mail of extension 
                    ...
                    $alert->DoMail($adminEmail,"Fleet Manager",$mailBody,"Car Booking Extension",$link);
