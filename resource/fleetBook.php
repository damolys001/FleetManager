...
$fleetUser  = "";  
$tripType="";
$startBookHrs = DateTime::createFromFormat('H:i', "07:00");
...
//echo $nowST->format("h:i A") ; echo " ".$startBookHrs->format("h:i A");

if($nowST< $startBookHrs || $nowST >$endBookHrs)
{
    echo "Booking is not possible at this time. Please come back by ".$startBookHrs->format("h:i A");
       exit;
}
...
{
     echo ("Maximum Hours allow per booking is ".$maxHour."hrs. Please contact General Service for clearification or exception. " );
    exit;
}
...
if ($userid=="") 
{
    echo ("Invalid user credentials. Please log out and re-login");
    exit;
}
...
//    exit;
//}
if (($startBookHrs >$myStartDateTime) || ($endBookHrs < $myStartDateTime))
{
     echo ("Booking is not available at this hour. You can only book between the hours of ". $startBookHrs->format("h:i A")." and ".$endBookHrs->format("h:i A")."  "); //$myStartDateTime->format("H:i").$startBookHrs->format("H:i")
    exit;
}

if (($datenow != $myStartDateTime->format("Y/m/d")) )
{
    if (($_SESSION["role"]== "1")|| ($_SESSION["role"]== "2")){}else 
    {
        if ($tripType =="Intra State")
    {
     echo ("You can only have Intra State booking for today. Please contact general services for clarification.");
     exit;
    }
    }
}
...
if (($datenow < $myEndDateTime->format("Y/m/d")) || ($datenow > $myEndDateTime->format("Y/m/d")))
{
    if (($_SESSION["role"]== "1")|| ($_SESSION["role"]== "2")){}else 
    {
        if ($tripType =="Intra State")
...
//check if hrs is fall between 30 min of an approve bookings
$queryBkHr = "select * from  bookinglog where createdForD ='".$datenow."' and fleet = '".$id."' and status = 'Approved' and 
...
///Check if user have an unrated completed trip
...
if ($driverName == "")
{
    ...
    $mailBody="<style>table {border-collapse: collapse;    width: 50%;} th, td { text-align: left; padding: 8px;} tr:nth-child(even){background-color: #f2f2f2}</style>
                    Dear Admin,<br/><br/>
Kindly note that a user $userName was prevent from booking the car detailed below. Because no driver was assigned<br/><br/>
...
<tr><td>Driver Phone:</td><td>$driverPhone<td></tr>
                    ...
                    ";
      $alert->DoMail($adminEmail,"Fleet Manager",$mailBody,"Car Booking Rejection: No Driver Assigned",$link);
    exit;
}
...
<tr><td>Driver Phone:</td><td>$driverPhone<td></tr>
                   ...
                   }else
                   {
                    ...
                    //$smsBody ="Hello $driverName, you have been schedule for a trip. Pick location:$depature, Destination:$destination, Time/Date:".$myStartDateTime->format('h:i A').". By: $userName ";
                     $smsBody ="You have been schedule for a trip. Pickup location:$depature, Destination:$destination, Time/Date:".$myStartDateTime->format('h:i A')." End:".$myEndDateTime->format('h:i A').". By: $userName";
                     //$driverPhone
...
Kindly note that the booking request for the car detailed below has been requested by $userName for your approval. You can login at http://fleetmanager.ensure.com.ng<br/><br/>
<table align='center' style='border-collapse: collapse;    width: 50%; tr:nth-child(even):background-color: #f2f2f2}'>
<tr><td>Car Make:</td><td> $carMakeModel<td></tr>
<tr><td>Car User:</td><td> $userName<td></tr>
<tr><td>Car Registration No: </td><td>$regNo<td></tr>
